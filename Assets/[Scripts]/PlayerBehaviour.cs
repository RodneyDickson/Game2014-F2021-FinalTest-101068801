...
public class PlayerBehaviour : MonoBehaviour
{
    ...
    public Transform shrinkingPlatformStartPoint;
    ...
    public Vector3 minPlatformSize = new Vector3(0.2f, 0.2f, 0.2f);
    public Vector3 maxPlatformSize = new Vector3(1.0f, 1.0f, 1.0f);
    ...
    void FixedUpdate()
    {
        ...
        // Checks for space being pressed to jump
        ...
        // Checks for A to be pressed to move LEFT
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        if (other.gameObject.CompareTag("Shrinking Platform"))
        {
            ...
            if (Vector3.Distance(other.transform.localScale, minPlatformSize) > 0.1f)
            {
                other.transform.localScale = maxPlatformSize;
            }
        }
    }
    ...
    private void OnCollisionEnter2D(Collision2D other)
    {
        ...
        if (other.gameObject.CompareTag("Shrinking Platform"))
        {
            ...
            if (Vector3.Distance(other.transform.localScale, minPlatformSize) < 0.1f)
            {
                sounds[(int)ImpulseSounds.PLATFORMDISAPPEAR].Play();
                Instantiate(objectToSpawn, other.transform.position, Quaternion.identity);
                sounds[(int)ImpulseSounds.PLATFORMSPAWN].Play();
            }
        }
    }
